...
class PhanAnhDaXuLyFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_phan_anh_da_xu_ly, container, false)
    ...
    private fun setupRecyclerView(view: View) {
        recyclerView = view.findViewById(R.id.rv_da_xu_ly)
            ...
            override fun onResponse(call: retrofit2.Call<gopymodel2>, response: retrofit2.Response<gopymodel2>) {
                if (response.isSuccessful) {
                    response.body()?.let { result ->
                        val filteredData = result.getData().filter { it.trangThai == "2" }
                        gopyAdapter =
                            AdminGopyAdapter(
                                filteredData
                            )
